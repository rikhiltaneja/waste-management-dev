...
class WorkerRecommendationService {
  ...
  async getRecommendedWorkers(options?: {
    ...
    try {
      ...
      const url = `${this.baseUrl}/workers-prediction/recommend${params.toString() ? '?' + params.toString() : ''}`;
      
      const response = await fetch(url);
      
      if (!response.ok) {
        throw new Error(`Failed to fetch recommended workers: ${response.statusText}`);
      }
  ...
  async assignTaskToWorker(request: AssignTaskRequest): Promise<AssignTaskResponse> {
    try {
      const response = await fetch(`${this.baseUrl}/workers-prediction/assign-task`, {
        method: 'POST',
        headers: {
  ...
  calculateTaskDifficulty(complaintDescription: string, complaintType?: string): number {
    const description = complaintDescription.toLowerCase();
    
    // Simple heuristic based on keywords
    if (description.includes('illegal dumping') || description.includes('construction waste')) {
      return 8; // High difficulty
    } else if (description.includes('overflowing') || description.includes('not collected')) {
      return 6; // Medium-high difficulty
    } else if (description.includes('sweeping') || description.includes('leaves')) {
