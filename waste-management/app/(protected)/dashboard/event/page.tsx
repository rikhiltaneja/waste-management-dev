...
import { CalendarPlus, Eye, Edit, Trash2, MapPin, Calendar, Clock, Users2Icon, TicketIcon, Grid3X3, List, LayoutGrid } from 'lucide-react'
import { useRouter } from 'next/navigation'
import { Button } from '@/components/ui/button'
import { Card } from '@/components/ui/card'
import { StatsCardGrid } from '@/components/ui/charts'
import { AddEventModal, EventFormData } from '@/components/modals/add-event-modal'
...
const CampaignPage = () => {
  ...
  const [viewingEvent, setViewingEvent] = useState<PhysicalTrainingEvent | null>(null)
    ...
    <SideBarLayout primaryAction={{
      ...
      <div className="p-4 md:p-6 space-y-4 md:space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-gray-900">Physical Training Events</h1>
            <p className="text-gray-600 mt-1 text-sm md:text-base">Manage and monitor training events for citizens and workers</p>
          </div>
        </div>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredEvents.map((event) => (
                    <tr key={event.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4">
                        <div className="text-sm text-gray-900">{formatDate(event.startDateTime)}</div>
                        {event.endDateTime && (
                          <div className="text-sm text-gray-500">to {formatDate(event.endDateTime)}</div>
                        )}
                      </td>
                      <td className="px-6 py-4">
                        ...
                        {event.locality && (
                          <div className="text-xs text-gray-500">{event.locality.name}</div>
                        )}
                      </td>
                      <td className="px-6 py-4">
        ...
        {viewMode === 'grid' && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {filteredEvents.map((event) => (
            <div onClick={() => handleViewEvent(event.id)} className='cursor-pointer'>
              <EventCard
                key={event.id}
                title={event.title}
        ...
        {filteredEvents.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="mt-1 text-sm text-gray-500">
              {filter === 'ALL' ? 'Get started by creating a new event.' : `No ${filter.toLowerCase()} events found.`}
            </p>
            {filter === 'ALL' && (
              <div className="mt-6">
      ...
      <AddEventModal
        ...
        initialData={viewingEvent ? {
          ...
          status: viewingEvent.status === 'COMPLETED' || viewingEvent.status === 'CANCELLED' ? 'ACTIVE' : viewingEvent.status,
          localityId: null
        } : undefined}
      />
    </SideBarLayout>
