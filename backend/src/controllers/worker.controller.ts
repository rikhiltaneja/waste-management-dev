...
export const createWorker = async (req: Request, res: Response) => {
  const { name, phoneNumber, email, localityId, workerType } = req.body;
  if (!name || !phoneNumber || !email || !localityId || !workerType)
    return res.status(400).json({ error: 'All fields required, including workerType' });
  ...
  const workerId: string = (req.body.workerId as string) ?? `W-${Date.now()}`;
...
export const updateWorkerIncentive = async (req: Request, res: Response) => {
  ...
  const worker = await prisma.worker.update({
    where: { id },
    data: { /* here you can store bonus as points or extra field in Worker */ },
  });
